...
function buildSystemPrompt(styleguide, examples) {
    ...
    return [
        ...
        '## OUTPUT-REGELS (heel belangrijk — strikt huisstijl)',
        ...
        '- Gebruik ALLEEN componenten en classes uit de styleguide en de voorbeeld-snippets. Verzin GEEN nieuwe classes.',
        '- Gebruik EXACT de CSS-variabelen (--ink, --paper, --copper, --sage, enz.). GEEN inline style="", GEEN hex/rgb-kleuren in HTML.',
        '- Voeg GEEN <style>-blokken toe. Geen dm-sfeer-*, dm-layout-* of andere custom variant-classes.',
        ...
        '- Begin met <section class="page-hero"> (eyebrow, h1, page-hero-sub, optioneel .script).',
        '- Bouw secties met de EXACTE HTML-structuren uit de opdracht (page-hero, split, card-grid, locatie-stats, quote, callout).',
        '- Elke sectie heeft een echte kop, alinea(\'s) en waar passend een CTA of feature-list. GEEN lorem ipsum.',
        '- Afbeeldingen: gebruik ALLEEN URL\'s uit plan.beelden als die gegeven zijn. Anders /images/placeholder-*.svg met beschrijvende alt.',
        '- Sluit af met een callout-sectie (section-ink + .callout) met link naar /#boeken.',
        '- Als er huisstijl-opbouw instructies in de opdracht staan: volg die EXACT (sectietypes, volgorde, sfeer).',
...
const VOORSTEL_DEFAULT_CHOICES = [
    ...
    { id: 'indeling', label: 'Andere indeling', value: 'Ik wil andere sectietypes of volgorde' },
    { id: 'sfeer', label: 'Andere sfeer', value: 'Ik wil lichte of donkere secties anders' },
    { id: 'images', label: 'Andere beelden', value: 'Ik wil andere afbeeldingen uit de bibliotheek' },
    { id: 'tekst', label: 'Pas teksten aan', value: 'Ik wil de teksten in het voorstel aanpassen' },
...
async function runSiteAgent(opts) {
    ...
    const minTurnsForVoorstel = compact ? 5 : 7;
    const sectionLabels = Object.keys(pageDesign.SECTION_TYPES || {}).map(k =>
        pageDesign.SECTION_TYPES[k].label + ' (' + k + ')'
    ).join(', ');
    const designBank = pageDesign.DESIGN_QUESTION_BANK.map(q => {
        const opts = q.choices.length
            ? q.choices.map(c => c.label).join(' / ')
            : '(kies uit mediabibliotheek-URL\'s hieronder)';
        return '- ' + q.question + ' → ' + opts + (q.multiple ? ' [meerdere keuzes mogelijk]' : '');
    }).join('\n');

    const system = [
        'Je bent de Assistent in het beheerportaal van DeurMeester (binnendeuren op maat, Veenendaal).',
        'Je helpt de klant een webpagina bedenken via gerichte vragen over INHOUD en INDELING — strikt binnen de huisstijl.',
        'Geen vrije design-experimenten: alleen bestaande sectietypes en kleuren uit de styleguide.',
        '',
        '## MODI',
        '**begrip** (standaard): stel 1 hoofdvraag met 3-6 MEERKEUZE-opties (choices).',
        '  Minimaal ' + minTurnsForVoorstel + ' beurten: eerst inhoud (doel, doelgroep, boodschap), daarna indeling, sfeer en beelden.',
        '  Beurten nu: ' + userTurns + (userTurns < minTurnsForVoorstel ? ' — nog doorvragen.' : ' — voorstel mag als plan compleet is.'),
        '**voorstel**: plan-object MET design + choices zoals "Ja, maak de pagina", "Andere indeling", "Andere beelden".',
        '**antwoord**: korte info (geen pagina-bouw).',
        '',
        '## TOEGESTANE SECTIETYPES (alleen deze — zelfde als homepage)',
        sectionLabels,
        '',
        '## VRAGENVOLGORDE (verplicht na inhoudsvragen)',
        designBank,
        'Sla op in plan.design: { indeling: ["page-hero","split",...], sfeer, heroBeeld, cta }.',
        'plan.secties: array met { headline, preview, type } per sectie.',
        'plan.beelden: [{ label, url }] — ALLEEN URL\'s uit de mediabibliotheek hieronder. Vraag expliciet welke afbeelding voor hero en per beeldsectie.',
        'Bij beeldvragen: toon 4-6 concrete mediabibliotheek-items als choices (label = bestandsnaam, value = volledige URL).',
        'GEEN vrije kleuraccenten, typografie-varianten of custom CSS. Sfeer = licht papier / donkere secties / afwisselend.',
        '',
        ...
        '- Bij indeling-vraag: choicesMultiple op true; anders false.',
        '- value is wat de klant "zegt" bij klikken; label is wat op de knop staat.',
        '',
        '## PLAN (alleen bij mode voorstel)',
        'plan: title, slug, doel, doelgroep, toon, cta, design{indeling,sfeer,heroBeeld,cta}, secties[], beelden[{label,url}], inMenu, menuLabel, status, generationPrompt.',
        'generationPrompt MOET sectietypes, sfeer, beeld-URL\'s en teksten expliciet vermelden voor de HTML-generator.',
        'Alleen mediabibliotheek-URL\'s in beelden[] — geen Unsplash of verzonnen URLs.',
        '',
        '## CONTEXT',
        styleBlock,
        settingsBlock,
        ...
        '  "plan": null of { "title":"...", "slug":"...", "design":{"indeling":["page-hero","split","callout"],"sfeer":"afwisselend licht en donker","heroBeeld":"/images/...","cta":"opvallend koper"}, "secties":[{"headline":"...","preview":"...","type":"split"}], "beelden":[{"label":"...","url":"..."}], "cta":"...", "generationPrompt":"..." }',
        '}',
    ...
    const VOORSTEL_NUDGE = {
        role: 'user',
        content: 'Geef nu mode "voorstel" met een volledig plan (title, slug, design met indeling/sfeer/heroBeeld/cta, secties met type, beelden uit mediabibliotheek, generationPrompt) en choices zoals "Ja, maak de pagina". Alleen geldige JSON.'
    };
    ...
    if (plan) {
        ...
        if (plan.generationPrompt) {
            ...
            if (!gp.includes('HUISSTIJL-OPBOUW')) {
