...
async function runSiteAgent(opts) {
    ...
    const system = [
        ...
        'Je helpt de klant een webpagina bedenken via korte vragen, toont een voorbeeldplan, en bouwt pas na bevestiging.',
        '',
        '## MODI',
        '**begrip** (standaard): stel 1 hoofdvraag met 3-5 MEERKEUZE-opties (choices). Minimaal 3 gespreksbeurten voor voorstel.',
        '  Beurten nu: ' + userTurns + (userTurns < 3 ? ' — nog doorvragen.' : ' — voorstel mag.'),
        '**voorstel**: rijk plan-object + choices zoals "Ja, maak de pagina", "Pas de titel aan", "Andere beelden", "Meer secties".',
        '**antwoord**: korte info (geen pagina-bouw).',
        '',
        '## CHOICES (verplicht bij begrip en voorstel)',
        '- choices: array van 3-5 objecten { "id": "a", "label": "korte knoptekst", "value": "volledige zin die als gebruikersantwoord telt" }',
        ...
        'plan: title, slug, doel, doelgroep, toon, cta, secties[], beelden[{label,url}], inMenu, menuLabel, status, generationPrompt.',
        ...
        '  "plan": null of { "title":"...", "slug":"...", "secties":[{"headline":"...","preview":"..."}], "beelden":[{"label":"...","url":"..."}], "cta":"...", "generationPrompt":"..." }',
        '}',
        'Nederlands. Geen em-dashes. Geen HTML in reply.'
    ].filter(Boolean).join('\n');

    const shouldVoorstel = userTurns >= 3;
    ...
    const VOORSTEL_NUDGE = {
        role: 'user',
        content: 'Geef nu mode "voorstel" met een volledig plan (title, slug, secties met headline/preview, beelden, cta, generationPrompt) en choices zoals "Ja, maak de pagina". Alleen geldige JSON.'
    };
